%% Maps test
% Homeostatic map
generator = MapProcessor(100,250,20,60,robot);
%%
[normAngle,normTorque,alphaH] = generator.getHomeostaticMap(60,@(x)awgn(heaviside(x),30,'measured'));
%%
...
text(105,50,'Subamortiguado');
text(200,50,'Sobreamortiguado')
text(155,50,'Crítico \rightarrow');
xlim(xVec)
ylim(yVec)
hold off
...
[sdAngle,sdTorque,alphaHR] = generator.getReducedHomeostaticMap(60,@(x)awgn(heaviside(x),30,'measured'));
%%
%Map of force
figure(1)
hold on
...
corrMap = generator.getCorrelationMap(60,@(x)awgn(heaviside(x),30,'measured'));
%%
%Map of correlation
figure(1)
hold on
sgtitle('Mapa correlación torca - ángulo')
...
hold off
%%
corrMapEnergy = generator.getEnergyCorrelationMap(60,@(x)awgn(heaviside(x),30,'measured'));
%%
%Map of correlation
figure(1)
hold on
